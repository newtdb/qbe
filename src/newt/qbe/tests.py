...
class QBETests(newt.db.tests.base.TestCase):
    ...
    def test_array(self):
        ...
        self.assertEqual(
            b"(state -> 'x')"
            b" && ARRAY['a', 'b', 'c']",
            self.qbe.sql(self.conn, dict(x=['a', 'b', 'c'])))
        self.assertEqual(
            b"(state -> 'x')",
    ...
    def test_integration(self):
        ...
        self.assertEqual(
            ['newt uses ZODB'],
            [o.text for o in where(qbe.sql(self.conn, dict(stars=4)))],
            )

        self.assertEqual(
            ['qbe is nearing release', 'We have two newt reviews',
             'newt uses ZODB'],
            [o.text for o in where(qbe.sql(self.conn, dict(stars=(None, 4)),
                                           order_by=['stars']))],
            )

        self.assertEqual(
            ['We have two newt reviews', 'newt uses ZODB'],
            [o.text for o in where(qbe.sql(self.conn, dict(stars=(3, 4)),
                                           order_by=['stars']))],
            )

        self.assertEqual(
            ['We have two newt reviews', 'newt uses ZODB'],
            [o.text for o in where(qbe.sql(self.conn,
                                           dict(stars=(None, 4), path='/db'),
                                           order_by=['stars']))],
            )
